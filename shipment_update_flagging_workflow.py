...
from kpler_handler import KplerSession, KplerAuthenticationRequired
...
FAILURE_MARKER = Path(os.getenv("WORKER_STATE_DIR", str(Path(__file__).resolve().parent))) / ".worker_failed"
    ...
    if not sg_bound:
        ...
        for p in recent:
            bearing_to_sg = initial_bearing(p["geo"]["lat"], p["geo"]["lon"],
                                            SINGAPORE["lat"], SINGAPORE["lon"])
            ship_course   = p.get("course") if p.get("course") is not None else p.get("heading")
...
def send_telegram_alert(result: DiversionResult, flag_reverted: bool) -> None:
    """Send a transition; raise on failure so the sheet flag can be retried."""
    p = result.latest_ping or {}
    geo = p.get("geo") or {}
    if result.mode == "sg_bound":
        direction = "may be diverting away from Singapore"
    else:
        direction = "may be heading toward Singapore instead of its stated destination"
    title = "Diversion alert reverted" if flag_reverted else "Possible shipment diversion"
    text = (
        f"🚨 <b>{title}</b>\n\n"
        f"<b>Ship:</b> {escape(str(result.ship_name))} "
        f"(IMO: {escape(str(result.ship_id))})\n"
        f"<b>Cargo:</b> {escape(str(result.cargo or 'Unknown'))}\n"
        f"<b>Assessment:</b> {escape('No longer flagged' if flag_reverted else direction)}\n"
        f"<b>Score:</b> {result.score}/3 signals\n"
        f"<b>Latest position:</b> {escape(str(geo.get('lat', '?')))}, "
        f"{escape(str(geo.get('lon', '?')))}\n"
        f"<b>Speed:</b> {escape(str(p.get('speed', '?')))} kn\n"
        f"<b>Position timestamp:</b> {escape(str(p.get('receivedTime', '?')))} UTC"
    )
    if result.ais_gap_detected and result.ais_jump_info:
        gap = result.ais_jump_info
        text += f"\n<b>AIS gap:</b> {gap['gap_hours']} hours; position jump {gap['jump_nm']} nm"
    if DASHBOARD_URL:
        text += f'\n<a href="{escape(DASHBOARD_URL, quote=True)}">Open vessel dashboard</a>'

    if not TELEGRAM_BOT_TOKEN or TELEGRAM_BOT_TOKEN == "YOUR_BOT_TOKEN" or not TELEGRAM_CHANNEL_ID or TELEGRAM_CHANNEL_ID == "YOUR_CHANNEL_ID":
        raise RuntimeError("Telegram bot token or channel ID is not configured")
    ...
    try:
        ...
        # Do not include the request URL or response body: both may expose secrets.
        raise RuntimeError("Telegram delivery failed; verify bot and chat settings") from exc
    log.info("Telegram transition sent for %s", result.ship_name)
...
def is_flagged(value) -> bool:
    """Normalize checkbox values and blank cells returned by Google Sheets."""
...
def run() -> None:
    ...
    for row_idx, record in ships:
        ...
        if not kpler_id or not departure:
            ...
            failures.append(row_idx)
            continue
        if not (-90 <= dest_lat <= 90 and -180 <= dest_lon <= 180) or not record.get("Original_Dest_Lat") or not record.get("Original_Dest_Long"):
            log.error("  Skipping — invalid destination coordinates.")
            failures.append(row_idx)
            continue
        ...
        except KplerAuthenticationRequired:
            # All rows would fail; surface a single actionable error to the scheduler.
            raise
        except Exception as exc:
        ...
        if fresh_positions and len(fresh_positions) >= MIN_PINGS_REQUIRED:
            ...
            origin_lat = first_ping["geo"]["lat"]
            origin_lon = first_ping["geo"]["lon"]
            ...
            prev_flag = is_flagged(record.get("Diversion_Flag"))
            # A chokepoint can temporarily suppress a score. It is not evidence
            # that an earlier diversion has reverted.
            if result.near_chokepoint:
                log.info("  Holding prior flag while near %s", result.near_chokepoint)
                result.flagged = prev_flag
            try:
                latest_time = datetime.fromisoformat(fresh_positions[-1]["receivedTime"].replace("Z", "+00:00"))
                age_hours = (datetime.now(timezone.utc) - latest_time.astimezone(timezone.utc)).total_seconds() / 3600
            except (ValueError, TypeError, KeyError):
                age_hours = float("inf")
            if age_hours > 48:
                log.warning("  Latest position is %.1f hours old; holding prior flag", age_hours)
                result.flagged = prev_flag
                failures.append(row_idx)
            if DRY_RUN:
                log.info("  DRY RUN: row %d old=%s new=%s; no writes or alerts", row_idx, prev_flag, result.flagged)
                continue
            if result.flagged != prev_flag:
                try:
                    send_telegram_alert(result=result, flag_reverted=not result.flagged)
                except Exception as exc:
                    log.error("  Alert failed for %s: %s", ship_name, exc)
                    failures.append(row_idx)
                    # Preserve the previous flag so the next run retries the alert.
                    result.flagged = prev_flag
                else:
                    flagged_count += 1
            record["Coord_Trace"] = fresh_positions
            ...
            log.warning("  Insufficient fresh pings for row %d; preserving previous flag", row_idx)
            failures.append(row_idx)

    log.info(
        "═══ Done. %d transition alert(s) for %d ship(s). ═══",
        flagged_count, len(ships),
    )
    if failures:
        raise RuntimeError(f"Worker had errors on sheet rows {sorted(set(failures))}; check the log")


def notify_worker_status(message):
    """Best-effort health notice; failure details and credentials stay in logs."""
    if DRY_RUN or not TELEGRAM_BOT_TOKEN or TELEGRAM_BOT_TOKEN == "YOUR_BOT_TOKEN" or not TELEGRAM_CHANNEL_ID:
        return False
    try:
        response = requests.post(
            f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
            json={"chat_id": TELEGRAM_CHANNEL_ID, "text": message}, timeout=10,
        )
        return response.ok and bool(response.json().get("ok"))
    except (requests.RequestException, ValueError):
        log.error("Could not send worker health notice")
        return False
...
if __name__ == "__main__":
    try:
        ...
        log.exception("Vessel-monitoring worker failed")
        if not FAILURE_MARKER.exists() and notify_worker_status(
            "⚠️ Vessel-monitoring worker failed. Alerts may be delayed. Check the worker log."
        ):
            FAILURE_MARKER.touch()
        raise SystemExit(1)
    else:
        if FAILURE_MARKER.exists():
            notify_worker_status("✅ Vessel-monitoring worker completed successfully again.")
